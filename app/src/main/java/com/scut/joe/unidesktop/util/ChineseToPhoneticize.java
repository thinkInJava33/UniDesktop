...
/**
 * 汉字转成拼音,来自android4.2源码
 */
public class ChineseToPhoneticize {
    private static final String TAG = "ChineseToPhoneticize";
    ...
    private static ChineseToPhoneticize sInstance;
    ...
    protected ChineseToPhoneticize(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    public static ChineseToPhoneticize getInstance() {
        synchronized (ChineseToPhoneticize.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINESE)) {
                    ...
                    sInstance = new ChineseToPhoneticize(true);
                    return sInstance;
                }
            }
            Log.w(TAG, "There is no Chinese collator, ChineseToPhoneticize is disabled");
            sInstance = new ChineseToPhoneticize(false);
            return sInstance;
        }
    }
